...
Proportion of Candidates' Pro-Campaign Ads Run from Oct 2015 - Dec 2016 
```{r}
gov50ads |> 
  group_by(candidates) |>
  summarize(n = n()) |>
  ...
  labs(
    ...
    x = "Candidate")
```
Proportion of Hillary Clinton Ads in Different Districts

```{r}
gov50ads <- gov50ads |> 
  mutate(Hillad = case_when(candidates == "Hillary Clinton" ~ "1",
                            candidates != "Hillary Clinton" ~ "0"),
...
gov50ads |>
  ...
  geom_col() + 
  labs(title = "Proportion of Campaign Ads Belonging to Hillary Clinton Across States",
...
```
RESULT SUMMARY: 
In the scatterplot, we can see that as Hillary Clinton's proportion of campaign ads increased, her share of the vote actually decreased. According to the line of best fit model, the intercept is 79.88. That means that when Clinton ads make up 0% of the percentage of campaign ads, she has 77.88% of the vote for that district. The slope is -0.405. That is, for every increase in 1 percentage point of the campaign ads that Clinton's ads make up, her share of the vote decreases by 0.405 percentage points. 
...
Another limitation is that my study may not adequately address the fact that a district with negative pre-existing notions of Hillary Clinton may be targeted with a greater percentage of ads in order to improve that perception. The improvement suggested in the previous paragraph may help account for that. 